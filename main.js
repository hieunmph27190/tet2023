...
$(function () {
    ...
    var loiChucs = [
        "Mong rằng năm mới sẽ mang lại cho bạn nhiều cơ hội mới để phát triển và thành công trong cuộc sống và sự nghiệp của mình.",
        "Chúc bạn có một năm mới đầy niềm vui và tiến bộ, vượt qua mọi thách thức và đạt được mục tiêu của mình.",
        "Chúc bạn và gia đình có một năm mới tràn đầy niềm vui và kỷ niệm đẹp, luôn gắn kết và ủng hộ lẫn nhau.",
        "Mong rằng năm mới sẽ mang lại cho bạn nhiều trải nghiệm mới, kỷ niệm đáng nhớ và hạnh phúc bất ngờ.",
        "Chúc bạn có một năm mới thú vị và đầy sự phấn khích, khám phá những điều mới mẻ và thú vị trong cuộc sống.",
        "Hãy bắt đầu một hành trình mới với lòng quyết tâm và sự đam mê, để bạn có thể đạt được mọi ước mơ của mình.",
        "Chúc bạn và gia đình có một năm mới đầy bình an và hạnh phúc, luôn được bảo vệ và phù hộ từ những điều xấu xa.",
        "Mong rằng năm mới sẽ mang lại cho bạn nhiều cơ hội để gặp gỡ và kết bạn với những người tuyệt vời trong cuộc đời của mình.",
        "Chúc bạn có một năm mới thành công rực rỡ và đầy sức sống, để bạn có thể tỏa sáng và góp phần vào sự phát triển của cộng đồng và xã hội.",
        "Hãy sống mỗi ngày với lòng biết ơn và hạnh phúc, để bạn có thể tận hưởng mọi khoảnh khắc và giá trị cuộc sống."
    ];
    ...
    function loop() {
        update();
        draw();
        xxx = setTimeout(loop, 1000 / 60); // Gọi lại hàm loop sau mỗi frame (tần suất khoảng 60fps)
    }
    function randomElements(arr, n) {
        var result = [];
        var len = arr.length;

        // Nếu số lượng phần tử cần chọn lớn hơn hoặc bằng độ dài của mảng, trả về toàn bộ mảng
        if (n >= len) {
            return arr;
        }

        // Lặp qua mảng để chọn ngẫu nhiên các phần tử
        while (result.length < n) {
            var index = Math.floor(Math.random() * len); // Chọn ngẫu nhiên một index trong khoảng từ 0 đến len - 1
            if (!result.includes(arr[index])) { // Kiểm tra xem phần tử đã được chọn trước đó chưa
                result.push(arr[index]); // Thêm phần tử vào mảng kết quả
            }
        }
    ...
    let elapsedTime = 0;
    const stopTime = 10 * 1000; // 10 giây, chuyển đổi sang milliseconds
    var abc = true;
    function startLoop() {
        setTimeout(() => {
            ...
            var randomElementsArray = randomElements(loiChucs, 3);
            let divLoiChucs = document.createElement("div");
            randomElementsArray.forEach(element => {
                let newDiv = document.createElement("div");
                // Thêm nội dung cho thẻ div
                newDiv.innerHTML = element;
                newDiv.style.fontSize = "24px";
                newDiv.style.color = "wheat";
                newDiv.style.margin = "5px";
                divLoiChucs.appendChild(newDiv);
            });
            
            divLoiChucs.id="loiChucs";
            document.body.appendChild(divLoiChucs);
           
            setTimeout(() => {
                divLoiChucs.innerHTML = ` <div id="right-desposit" class="right-desposit">
              <div class="show-banking-ul">
                <div class="vietcombank">
                  <p>Lì xì thông qua ngân hàng <span style="color: #11b89b; font-weight: 700; font-size: 13px">BIDV</span></p>

                  <img src="./asset/abc.png" alt="">
                </div>
                <div class="review-bank" style=" flex-wrap: wrap;">
                  <strong>Trao đi yêu thương nhân lại yêu thương :) </strong>
                  <br>
                  <label>Ngân hàng: <span style="color:#23e6a8; font-weight:800">BIDV</span> | STK nhanh: <span style="color:#23e6a8; font-weight:800">hieuit88</span> </label>
                  <br>
                  <label>Nội dung chuyển khoản: <span style="color:#23e6a8; font-weight:800">Tên người chuyển + Ngân hàng + Stk + Lời chúc</span> </label>
                  <br>
                  <br>
                  <strong>Mọi người lưu ý:</strong>
                  <br>
                  <span>Nội dung chuyển khoản vui lòng điền đúng để nhận lại yêu thương</span>
                  <br>
                </div>
              </div>
            </div> `;
                divLoiChucs.style.display="flex";
                divLoiChucs.style.justifyContent = "center";
            },10000)
